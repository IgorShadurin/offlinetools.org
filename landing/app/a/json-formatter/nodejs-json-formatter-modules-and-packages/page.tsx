    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <ol className="list-decimal pl-6 space-y-2 my-4">
          ...
          <li>
            <code>space</code> (Optional): A `string` or `number` value that's used to insert white space into the output JSON string for readability purposes.
          </li>
        </ol>
        <p>
...
// }`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Function:</strong> Called for each key/value pair. You can return the value to include it, or `undefined` to exclude it.
          </li>
          <li>
            <strong>Array:</strong> An array of strings or numbers that specifies which properties of the object should be included in the JSON string.
          </li>
        </ul>
...
// }`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>No Key Sorting:</strong> It does not provide an option to sort object keys alphabetically or in any specific order, which can be crucial for diffing large JSON files or ensuring consistent output.
          </li>
          <li>
            <strong>Performance on Large Data:</strong> For extremely large JSON objects, `JSON.stringify` can consume significant memory as it needs to hold the entire object in memory before converting it to a string.
          </li>
          <li>
            <strong>Error Handling:</strong> It throws errors on circular references or other non-JSON-serializable values, but lacks more sophisticated validation during the process.
          </li>
          <li>
            <strong>Limited Formatting Options:</strong> Beyond indentation, there are no options for custom spacing, wrapping, or other complex formatting rules.
          </li>
        </ul>
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p>
            This package is specifically designed to produce stable JSON output by sorting object keys alphabetically. This is invaluable for tasks like caching, hashing, or diffing where byte-for-byte consistency is required.
          </p>
...
// Note how keys are sorted alphabetically at each level.`}
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p>
            Part of the larger `js-beautify` library, this module offers more configurable options for formatting JSON, aligning values, controlling array indentation, etc.
          </p>
          <h5 className="text-base font-medium mt-2">Example Usage (Conceptual):</h5>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto text-sm">
            <pre>
...
// (Exact output depends on version and options)`}
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p>
            While primarily a code formatter, Prettier has excellent support for JSON. It's typically used as a development tool or pre-commit hook rather than directly formatting JSON within application logic at runtime, but it's the go-to tool for ensuring consistent JSON style across a project.
          </p>
          <h5 className="text-base font-medium mt-2">Usage:</h5>
          <p>
            Install Prettier (`npm install --save-dev prettier`) and run it from the command line or integrate it into your IDE/build process:
          </p>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto text-sm">
            <pre>
...
prettier --write "path/to/directory/**/*.json"`}
         ...
         <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p>
            For working with extremely large JSON files that don't fit into memory, streaming libraries are essential. While primarily parsers, some also offer transformation or stringification capabilities that can format data as it's processed, avoiding the need to load the entire file. These are more complex than simple formatters but necessary for scale.
          </p>
          <h5 className="text-base font-medium mt-2">Usage Concept:</h5>
          <p>
            Read the JSON file as a stream, pipe it through a parser/transformer stream that applies formatting rules, and write the output to a new stream or file.
          </p>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto text-sm">
            <pre>
...
// This approach avoids loading the entire 'large_input.json' into memory.`}
          ...
          <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
            Note: Implementing advanced formatting like deep indentation with simple streaming can be complex and might require buffering certain structures. Streaming is primarily for memory efficiency on large files.
          </p>
        </div>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Simple Pretty-Printing:</strong> For basic readability, use `JSON.stringify(data, null, 2)` or `JSON.stringify(data, null, '\t')`. This is the most performant and requires no external dependencies.
          </li>
          <li>
            <strong>Stable Output / Diffing:</strong> If you need consistent key ordering for caching, hashing, or comparing JSON structures, use a package like `json-stable-stringify`.
          </li>
          <li>
            <strong>Advanced Formatting Options:</strong> If you require fine-grained control over indentation, spacing, or other visual styles, explore libraries like `js-beautify`.
          </li>
          <li>
            <strong>Code Style Enforcement:</strong> For maintaining consistent JSON formatting across a codebase in a development setting, integrate a tool like Prettier.
          </li>
          <li>
            <strong>Large Files / Memory Constraints:</strong> When dealing with JSON files that exceed available memory, use streaming parsers/formatters like `jsonstream`.
          </li>
        </ul>
