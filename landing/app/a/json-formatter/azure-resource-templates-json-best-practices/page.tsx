...
export const metadata: Metadata = {
  ...
  description: "Learn best practices for writing robust and maintainable Azure Resource Manager (ARM) templates using JSON.",
};
...
&rbrace;`}
        ...
        <p>
          Use the <code>description</code> property to explain the purpose and expected value of each parameter. This is crucial documentation for anyone using your template.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
        ...
        <p>
          For parameters with a limited set of valid inputs (e.g., SKUs, locations), use the <code>allowedValues</code> property to prevent errors during deployment.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
...
&rbrace;`}
        ...
        <p>
          When you need to deploy multiple instances of the same resource type (e.g., several VMs, multiple subnets in a VNet), use the <code>copy</code> property on the resource definition. This is far better than copying and pasting resource blocks.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
        ...
        <p>
          Use <code>copy</code> within a resource definition (e.g., on the <code>properties</code> of a VNet subnet array) to create multiple child items within a single resource.
        </p>
...
&rbrace;`}
        ...
        <p>
          Every resource definition must specify an <code>apiVersion</code>. This version corresponds to a REST API version for that resource type.
        </p>
         <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
        ...
        <p>
          ARM deploys resources in parallel by default. Use the <code>dependsOn</code> property to specify that a resource must be deployed only after other resources are successfully created. This is critical for resources that rely on others, like a Virtual Machine needing a Network Interface, or a Web App needing an App Service Plan.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
          <Check className="inline-block text-green-500" /> Use Resource Names or Resource IDs
        </h3>
        <p>
          The <code>dependsOn</code> value is an array of resource names or resource IDs. Using resource IDs generated by functions like <code>resourceId()</code> is generally recommended for robustness, especially when dealing with resource types in different namespaces or nesting.
        </p>
         <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
          <Lightbulb className="inline-block text-yellow-500" /> Implicit Dependencies
        </h3>
        <p>
          Many ARM functions (like <code>reference()</code> or <code>resourceId()</code>) create implicit dependencies. If Resource B uses <code>reference(resourceA)</code>, ARM understands that Resource A must be deployed before Resource B. While implicit dependencies are often sufficient, explicit <code>dependsOn</code> adds clarity and can sometimes resolve complex ordering issues.
        </p>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
&rbrace;`}
        ...
        <p>
          ARM templates provide a rich set of built-in functions (like <code>concat</code>, <code>resourceId</code>, <code>reference</code>, <code>parameters</code>, <code>variables</code>, <code>union</code>, <code>length</code>, etc.) to construct values dynamically.
        </p>
         <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
          <Check className="inline-block text-green-500" /> Understand Their Purpose
        </h3>
        <p>
          Familiarize yourself with commonly used functions. For example, <code>resourceId()</code> is used to get the full ID of a resource, and <code>reference()</code> is used to get the runtime state (properties) of a deployed resource.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
          <Info className="inline-block text-blue-500" /> Case-Insensitivity
        </h3>
        <p>
          Template functions and their arguments are case-insensitive. <code>[parameters('name')]</code> is the same as <code>[PARAMETERS('NAME')]</code>.
        </p>
...
`}
        ...
        <p>
          Use the <code>condition</code> property on a resource definition to specify whether that resource should be deployed or not based on a boolean expression. This is useful for deploying certain resources only in specific environments or based on parameter inputs.
        </p>
...
&rbrace;`}
        ...
        <p>
          Consider using <a href="https://docs.microsoft.com/azure/azure-resource-manager/templates/template-specs" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">Template Specs</a> to store and share your templates securely within Azure, making linked templates easier to reference.
        </p>
        ...
        <p>
          Often, names are constructed using parameters (like environment or project code) and variables (for base names and suffixes) using the <code>concat</code> function.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
        ...
        <p>
          Use functions like <code>reference()</code> or <code>resourceId()</code> to get the properties of deployed resources and expose them as outputs.
        </p>
        <h3 className="text-xl font-semibold mt-4 flex items-center gap-2">
          <Info className="inline-block text-orange-500" /> Secure Sensitive Outputs
        </h3>
        <p>
          Mark sensitive outputs with the <code>&quot;type&quot;: &quot;securestring&quot;</code> property. Their values will be masked in deployment logs.
        </p>
...
&rbrace;`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>Using the <code>metadata</code> property within parameters, variables, resources, or outputs.</li>
          <li>Adding a top-level object property like <code>_comments</code> or <code>explanation</code> (though this adds non-standard properties).</li>
        ...
        <p>
          The <a href="https://docs.microsoft.com/azure/azure-resource-manager/templates/deploy-what-if" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">What-If operation</a> shows you what changes ARM will make to your environment without actually deploying. This is invaluable for catching unintended consequences.
        </p>
