  ...
  return (
    <article className="prose dark:prose-invert max-w-none">
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li><a href="https://prettier.io/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">Prettier</a> (supports JSON along with many other formats)</li>
        <li><a href="https://stedolan.github.io/jq/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">jq</a> (a command-line JSON processor that can also format)</li>
...
"tags": ["electronics", "gadget" ]
  ...
  "tags": [
    ...
    "gadget"
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">A formatter ensures everyone's code looks like the second example.</p>
      </div>
      ...
      <p>
        <strong>Purpose:</strong> Catch formatting issues *before* the code is even committed. This is the earliest point and saves CI/CD time.
      </p>
      <p>
        <strong>How:</strong> Use tools like <a href="https://pre-commit.com/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">pre-commit</a> (a framework) or configure Git hooks manually. The hook runs the formatter on staged files, either fixing them automatically or failing the commit if they are not formatted correctly.
      </p>
      <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
        <h4 className="font-semibold mb-2">Example <code>.pre-commit-config.yaml</code> (using Prettier):</h4>
        <pre><code className="language-yaml">{`repos:
...
`}</code></pre>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">This configuration runs Prettier on specified file types, including JSON, before allowing a commit.</p>
      </div>
      ...
      <p>
        <strong>Purpose:</strong> Ensure that no unformatted code makes it past the CI pipeline, even if pre-commit hooks were bypassed or misconfigured. This acts as the final gate before merging.
      </p>
      <p>
        <strong>How:</strong> Add a step in your CI configuration (e.g., GitHub Actions, GitLab CI, Jenkins) that runs the formatter in "check" or "diff" mode. This mode exits with a non-zero status if files are not formatted correctly, causing the CI build to fail.
      </p>
      <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
        <h4 className="font-semibold mb-2">Example CI Step (using Prettier check mode):</h4>
        <pre><code className="language-yaml">{`name: CI Pipeline
...
`}</code></pre>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">This GitHub Actions step will fail the build if <code>prettier --check</code> finds any formatting inconsistencies in JSON or YAML files.</p>
      </div>
      ...
      <p>
        <strong>Purpose:</strong> Ensure final build artifacts or configuration files are formatted correctly before deployment.
      </p>
      <p>
        <strong>How:</strong> Typically, formatting is a development/build concern. Running a formatter at the release stage might be necessary if files are generated or modified during the build in a way that could mess up formatting. However, it's usually better to ensure files are formatted earlier. You might use tools like <code>jq</code> here to process/format generated JSON config files.
      </p>
      <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
        <h4 className="font-semibold mb-2">Example: Formatting a Generated Config with jq</h4>
        <pre><code className="language-bash">{`# Assume config.json was generated or modified
...
`}</code></pre>
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Using <code>jq .</code> is a common way to re-indent and format JSON from the command line.</p>
      </div>
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li><strong>Developers:</strong> Don't have to manually format or argue about style. They can configure their IDEs to format on save, and trust the CI/CD check as a safety net.</li>
        <li><strong>Code Reviewers:</strong> Reviews focus on logic and content, not whitespace or style issues, leading to faster, more effective reviews.</li>
        <li><strong>Operations/DevOps:</strong> Ensures configuration files or data artifacts used in deployment are consistent and easy to read/debug if necessary.</li>
        <li><strong>Project Managers:</strong> Reduced friction and time spent on style arguments, leading to smoother development cycles.</li>
      </ul>
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li><strong>Tool Selection:</strong> Choose a formatter that supports JSON well and ideally other formats you use. Prettier is a popular choice for its multi-format support and opinionated style.</li>
        <li><strong>Configuration:</strong> Agree on formatting rules (indentation size, etc.) and configure the formatter and CI/CD steps consistently. Store configuration files (e.g., <code>.prettierrc</code>) in the repository.</li>
        <li><strong>Legacy Code:</strong> Applying formatting to a large existing codebase might result in a single, massive commit with only formatting changes. It's often best to do this as a dedicated effort or apply formatters incrementally if possible.</li>
        <li><strong>Educating the Team:</strong> Ensure all developers understand why formatting is important and how to use pre-commit hooks and IDE integrations to avoid CI failures.</li>
        <li><strong>Performance:</strong> For extremely large repositories, running a formatter on every file on every commit/push might take time. Configure tools to run only on changed files (like pre-commit hooks do) or target specific directories/file patterns in CI.</li>
      </ul>
