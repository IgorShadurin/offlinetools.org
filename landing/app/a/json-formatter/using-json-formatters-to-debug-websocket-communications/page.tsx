...
export const metadata: Metadata = {
  ...
  description:
    "Learn how to use JSON formatters to effectively debug the data exchanged over WebSocket connections.",
};
    ...
    "settings": {
      ...
      "notifications": true
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>Paste it into an online JSON formatter website (e.g., `jsonformatter.curiousconcept.com`, `jsonlint.com`, etc.) or a desktop application.</li>
          <li>The tool will instantly format and validate the JSON.</li>
        </ul>
        <p>
          <strong>Tip:</strong> Be mindful of pasting sensitive data into online tools. For production debugging or sensitive information, prefer offline tools or your browser's built-in capabilities.
        </p>
...
try {
  ...
  // Handle invalid JSON errors
        ...
        <p>
          This code snippet shows the core logic: <code>JSON.parse()</code> converts the string to a JavaScript object, and <code>JSON.stringify()</code> with the <code>null, 2</code> arguments converts it back to a nicely indented string. You could use this logic within a UI component that receives WebSocket messages and displays them in a <code>&lt;pre&gt;&lt;code&gt;</code> block.
        </p>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Look for Message Type:</strong> Formatted JSON makes it easy to spot the `eventType` or similar fields at the root level to quickly understand the message's purpose.</li>
          <li><strong>Inspect Payloads:</strong> Drill down into nested `payload` objects to find the specific data you need to verify.</li>
          <li><strong>Compare Messages:</strong> Copy formatted messages to a diff tool to see exactly what changed between two subsequent messages.</li>
          <li><strong>Validate Syntax:</strong> Many formatters also act as validators, catching syntax errors in the JSON. This is crucial for identifying issues with the sending client or server.</li>
          <li><strong>Handle Errors:</strong> Be prepared for non-JSON messages (like plain text errors or malformed data) and handle them gracefully in any custom formatting logic.</li>
        </ul>
        ...
        <p>
          Debugging WebSocket communications involving JSON doesn't have to be a struggle with raw, unreadable strings. By leveraging browser developer tools, online/offline formatters, browser extensions, or even integrating programmatic formatting using <code>JSON.stringify(obj, null, 2)</code> into your debugging workflow, you can quickly transform opaque messages into clear, structured, and easily inspectable data. This significantly speeds up the process of understanding message content, identifying data discrepancies, and resolving issues in your real-time applications.
        </p>
      </div>
    </>
  );
