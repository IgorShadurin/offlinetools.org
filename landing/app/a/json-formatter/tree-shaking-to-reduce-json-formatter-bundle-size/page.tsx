  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <p>
          Modern JavaScript bundlers like Webpack (v2+), Rollup, and Parcel support tree shaking. They analyze the <code>import</code> and <code>export</code> statements (ES Modules syntax) to understand the dependencies between different parts of your code. If a module exports multiple functions or variables, but your application only imports and uses a subset of them, tree shaking ensures only the used parts make it into the final output.
        </p>
        ...
        <ol className="list-decimal pl-6 space-y-2 my-4">
          <li>
            <strong>Use ES Module Syntax:</strong> Use <code>import</code> and <code>export</code> statements instead of CommonJS <code>require()</code> and <code>module.exports</code>. ES Modules allow for static analysis, meaning bundlers can determine imports and exports without executing the code.
          </li>
          <li>
            <strong>Design with Side Effects in Mind:</strong> Avoid top-level code in modules that has side effects (like modifying global objects or logging) unless necessary. If a module *must* have side effects (though this is rare for utility libraries like formatters), hint to the bundler using the <code>"sideEffects": false</code> property in the library's <code>package.json</code>, or specify which files *do* have side effects.
          </li>
          <li>
            <strong>Export Functionality Granularly:</strong> Instead of exporting a single large object or function, export individual functions and components separately.
          </li>
        </ol>
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Even if you only use <code>prettyPrint</code>, <code>minify</code> might be included.</p>
...
`}
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">Bundler can potentially exclude <code>minify</code> if not imported.</p>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto">
            <pre>
              {`// json-utils-es.js
...
`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Choosing Libraries That Support It:</strong> Check the library's documentation or its <code>package.json</code> (look for <code>"module"</code> or <code>"exports"</code> fields pointing to ES Module files).
          </li>
          <li>
            <strong>Using Named Imports:</strong> Always use specific named imports (e.g., <code>import &#x7b; prettyPrint &#x7d; from 'json-formatter-lib'</code>) rather than default imports or importing the entire library object (e.g., <code>import formatterLib from 'json-formatter-lib'</code>, unless the default export is explicitly designed to be small or composed of other tree-shakable parts).
          </li>
          <li>
            <strong>Configuring Your Bundler:</strong> Ensure your Webpack, Rollup, or Parcel configuration has tree shaking enabled (it's often enabled by default in production modes). Ensure you're not using plugins that might accidentally break tree shaking (e.g., some older Babel configurations).
          </li>
        </ul>

        <h3 className="text-xl font-semibold mt-6">Code Example: Using a Tree-Shakable Library</h3>
        <p>Imagine a hypothetical library <code>fancy-json-formatter</code>:</p>

        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <h4 className="text-lg font-medium mb-2"><code>fancy-json-formatter</code> Library Structure (Conceptual)</h4>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto">
            <pre>
...
`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Dynamic Imports:</strong> Code that is imported dynamically (e.g., using <code>import()</code>) is typically not tree-shaken in the main bundle, but rather split into a separate chunk. This is often desired behaviour.
          </li>
          <li>
            <strong>Side Effects:</strong> If a module has unavoidable side effects and is marked as having them, the entire module might be included even if not all exports are used.
          </li>
          <li>
            <strong>Bundler Configuration:</strong> Tree shaking must be correctly configured and not accidentally disabled by other settings or plugins.
          </li>
          <li>
            <strong>Library Design:</strong> The library itself must be written in a way that facilitates tree shaking (using ES Modules, granular exports). A poorly structured library might still include a lot of unused code even with tree shaking enabled.
          </li>
        </ul>
