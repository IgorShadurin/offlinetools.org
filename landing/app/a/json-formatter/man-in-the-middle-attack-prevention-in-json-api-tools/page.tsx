  ...
  return (
    <>
      <h1 className="text-3xl font-bold mb-6">
        <Network className="inline-block mr-2 text-blue-600" size={30} /> Preventing Man-in-the-Middle Attacks in JSON APIs
      </h1>
      ...
      <div className="space-y-8">
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-4"><Lock className="inline-block mr-2 text-green-600" size={24} /> The Foundation: HTTPS and TLS</h2>
          <p>
          ...
          <ul className="list-disc pl-6 space-y-2 mt-4">
            <li>
              <strong>Encryption:</strong> It scrambles the data being sent, making it unreadable to anyone who intercepts the traffic. This prevents eavesdropping.
            </li>
            <li>
              <strong>Authentication:</strong> It uses digital certificates to verify the identity of the server. This ensures that the client is communicating with the legitimate API server, not an imposter.
            </li>
          </ul>
          <p>
          ...
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
            ...
            <ul className="list-disc pl-6 text-sm">
              ...
              <li>Support strong TLS versions (e.g., TLS 1.2 or 1.3) and disable older, vulnerable versions (SSLv2, SSLv3, TLS 1.0, TLS 1.1).</li>
          ...
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
            ...
            <ul className="list-disc pl-6 text-sm">
              ...
              <li>You are not disabling certificate validation (e.g., using `--insecure` with `curl` or setting `rejectUnauthorized: false` in Node.js `https` module for production).
              </li>
            </ul>
...
*/`}
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-4"><Shield className="inline-block mr-2 text-blue-600" size={24} /> Advanced Defense: Certificate Pinning</h2>
          <p>
          ...
          <ul className="list-disc pl-6 space-y-2 mt-4">
            <li>
              <strong>How it works:</strong> The client embeds or knows the expected certificate's public key or hash beforehand. When establishing a TLS connection, after receiving the server's certificate, the client verifies that the certificate (or its public key) matches the pinned value, in addition to the standard CA validation.
            </li>
            <li>
              <strong>Considerations:</strong> Pinning adds complexity. When the server's legitimate certificate changes (e.g., renewal), all clients with the old pinned value will fail to connect until they are updated. Implement pinning with a backup key or implement a robust update mechanism.
            </li>
          </ul>
          <p>
        ...
        <section>
          <h2 className="text-2xl font-semibold mb-4"><Key className="inline-block mr-2 text-indigo-600" size={24} /> Beyond Transport: API Keys and Authentication</h2>
          <p>
          ...
          <ul className="list-disc pl-6 space-y-2 mt-4">
            <li>
              <strong>API Keys:</strong> Static tokens identifying a calling application or user. Should be treated as secrets and ideally transmitted securely (always over HTTPS). Can be passed via headers (`X-API-Key`), query parameters (less recommended due to logging), or body.
            </li>
            <li>
              <strong>OAuth 2.0 / OpenID Connect:</strong> More complex, token-based authentication flows providing more security features like scopes, refresh tokens, and separation of concerns.
            </li>
            <li>
              <strong>Session Tokens / JWTs:</strong> Used after initial authentication (e.g., login) to maintain state. Must be protected and have appropriate expiration times.
            </li>
          </ul>
          <p>
...
.catch(error => console.error('Error:', error));`}
            ...
            <p className="text-sm mt-2">
              <AlertCircle className="inline-block mr-1 text-yellow-600" size={16} /> Ensure API keys and tokens are stored securely on the client side and not exposed in public code or logs.
            </p>
          </div>
        </section>

        <section>
          <h2 className="text-2xl font-semibold mb-4"><Code className="inline-block mr-2 text-purple-600" size={24} /> Data Integrity and Validation</h2>
          <p>
          ...
          <ul className="list-disc pl-6 space-y-2 mt-4">
            <li>
              <strong>Input Validation (Server Side):</strong> Thoroughly validate incoming JSON payloads against an expected schema (data types, required fields, value constraints). Reject any request that doesn't conform. Libraries like Zod, Yup, or JSON Schema validators can help.
            </li>
            <li>
              <strong>Output Validation (Client Side):</strong> While less common for standard JSON APIs, if the integrity is paramount and cannot solely rely on TLS, you *could* implement checks like verifying a digital signature included in the JSON payload (signed by the server using a private key the client can verify with a public key). This is complex and often unnecessary if TLS is properly implemented end-to-end, but relevant for highly sensitive or offline scenarios.
            </li>
            <li>
              <strong>Hashing:</strong> For critical data elements within the JSON, you could theoretically include a hash of specific fields, allowing the recipient to recompute the hash and verify it matches. Again, this adds complexity.
            </li>
          </ul>
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
    ...
    if (error instanceof z.ZodError) {
      ...
      res.status(500).json({ error: 'Internal server error' });
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-2 mt-4">
            <li>
              <strong>Browser-based Web Applications:</strong> Rely heavily on the browser's built-in TLS implementation and CA trust store. Ensure the API is always accessed via HTTPS. HSTS helps prevent downgrading to HTTP. Cookie security (`Secure`, `HttpOnly`, `SameSite` flags) is crucial for session management.
            </li>
            <li>
              <strong>Mobile Applications:</strong> Must use HTTPS. Strongly consider implementing Certificate Pinning for critical APIs. Use secure storage for API keys/tokens (e.g., device's keychain).
            </li>
            <li>
              <strong>Desktop Applications:</strong> Similar to mobile apps, use HTTPS and consider Certificate Pinning. Be mindful of how API keys/secrets are stored locally.
            </li>
            <li>
              <strong>Command-Line Tools (`curl`, custom scripts):</strong> Explicitly use `https://`. Avoid insecure flags (`--insecure`, `-k`). Be cautious about storing credentials in scripts or history. Configure the environment to use a trusted CA store.
            </li>
            <li>
              <strong>Server-to-Server Communication:</strong> Always use HTTPS. If connecting to third-party APIs, ensure the client library verifies certificates by default. Certificate Pinning might be appropriate for connections to critical dependencies.
            </li>
          </ul>
        </section>
