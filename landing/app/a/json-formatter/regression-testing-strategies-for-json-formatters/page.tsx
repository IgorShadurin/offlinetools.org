    ...
    <>
      ...
      <div className="space-y-6">
        <p>
          ...
          string representation. This formatting often involves adding whitespace, indentation, and line breaks to make the
          JSON human-readable. Ensuring that these formatters consistently produce correct and predictable output across
          updates is crucial. This is where <strong>Regression Testing</strong> comes in.
        </p>
...
&#x7d;`}
        ...
        <p>
          The key requirement is that the *parsed* structure of the output must be identical to the *parsed* structure of the input.
          The formatting (whitespace, newlines) can change, but the data and its hierarchy must be preserved.
        </p>
        ...
        <p>
          Even seemingly simple formatting logic can break. Changes to the codebase (refactoring, feature additions, library
          updates) can inadvertently introduce bugs that:
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>JSON with escaped characters in strings (e.g., `\"`, `\\`, `\/`, `\b`, `\f`, `\n`, `\r`, `\t`, `\uXXXX`).</li>
          <li>Numbers with decimals, exponents, and leading zeros (though JSON standard disallows leading zeros except for 0 itself).</li>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>JSON with duplicate keys in objects (behavior is implementation-defined in the standard, but common libraries handle the last one).</li>
        </ul>
...
`}
        ...
        <p>
          <strong>Pros:</strong> Straightforward to implement. Can catch exact formatting regressions (e.g., wrong number of spaces).
          <br />
          <strong>Cons:</strong> Very brittle. Any minor, potentially acceptable formatting change (like an extra newline at the end) will cause the test to fail. Requires maintaining exact expected output strings for every input.
        </p>
...
// expect(parsedOutput).toEqual(parsedInput); // Deep comparison of structures`}
        ...
        <p>
          <strong>Pros:</strong> Much more robust to formatting changes. Tests the core requirement: preservation of data structure.
          <br />
          <strong>Cons:</strong> Doesn't test the *formatting* aspect at all. It only verifies that the output is valid JSON and represents the same data. You might miss bugs where formatting is incorrect but the data is preserved (e.g., wrong indentation levels).
        </p>
...
`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>The output must not contain sequences like `,,`, `[&#x7b;`, `&#x7d;&quot;`, etc. (depending on your specific formatting rules).</li>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>Integrate with CI platforms (GitHub Actions, GitLab CI, Jenkins, etc.) to catch regressions before changes are merged.</li>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Test Runners:</strong> Jest, Mocha, Vitest.</li>
          <li><strong>Assertion Libraries:</strong> Built into test runners or separate like Chai.</li>
          <li><strong>JSON Parsers:</strong> `JSON.parse()` (built-in), specialized libraries for performance or error handling.</li>
          <li><strong>Deep Equality Checkers:</strong> Libraries like `lodash.isEqual` or built-in assertions in test runners (`toEqual`).</li>
          <li><strong>Property-Based Testing Libraries:</strong> `jsverify`, `fast-check`.</li>
          <li><strong>Mutation Testing Tools:</strong> Stryker Mutator.</li>
          <li><strong>JSON Schema Validators:</strong> To verify output against a schema if applicable.</li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Defining "Correct" Formatting:</strong> If there isn't a strict, single canonical output, exact string matching is difficult.</li>
          <li><strong>Handling Vendor Extensions:</strong> Some JSON implementations might support non-standard features (like comments), which complicates testing.</li>
          <li><strong>Performance:</strong> Testing with extremely large or complex JSON can be slow.</li>
          <li><strong>Generating Invalid JSON:</strong> Creating a diverse set of invalid JSON inputs that cover all possible syntax errors is tricky.</li>
        </ul>
