      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            ...
            <li>Numbers: <code>/-?\d+(\.\d+)?([eE][+-]?\d+)?/</code></li>
            <li>Keywords: <code>/\b(true|false|null)\b/</code></li>
            <li>Operators/Punctuation: <code>/[&#x7b;&#x7d;\[\],\:]/</code></li>
          </ul>
          <p>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <strong>Nesting Complexity:</strong> Regex struggles to correctly match elements within nested structures without complex lookaheads or recursion, which can be inefficient or impossible with standard regex engines.
            </li>
            <li>
              <strong>Context Sensitivity:</strong> A regex might match a string, but it cannot inherently know if that string is an object key or a value. This limits the ability to highlight keys differently.
            </li>
            <li>
              <strong>Error Handling:</strong> Malformed JSON can easily break regex patterns, leading to incorrect or incomplete highlighting.
            </li>
            <li>
              <strong>Performance:</strong> Complex regex can be slow, especially on large inputs, and performance can degrade unpredictably (catastrophic backtracking).
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>Structural tokens: <code>&#x7b;</code>, <code>&#x7d;</code>, <code>[</code>, <code>]</code>, <code>:</code>, <code>,</code></li>
            <li>Literal tokens: String (e.g., `"key"`), Number (e.g., <code>123</code>), Boolean (<code>true</code>, <code>false</code>), Null (<code>null</code>)</li>
...
{`[
  ...
  { type: 'BraceClose', value: '}' }
          ...
          <p>
            The parser can distinguish roles. For instance, a string token following a <code>&#x7b;</code> and followed by a <code>:</code> is an object key. A token following a <code>:</code> or within <code>[ ]</code> is a value.
          </p>
          <p>
  ...
  properties: [
    {
      ...
      value: { type: 'StringLiteral', value: 'Alice', original: '"Alice"' }
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <strong>Token-based:</strong> Iterate through the token stream. Assign CSS classes based on token types (`string`, `number`, `boolean`, `null`, `punctuation`, `whitespace`). This is a significant improvement over pure regex as it correctly identifies token boundaries even in complex cases.
            </li>
            <li>
              <strong>AST-based:</strong> Traverse the AST. Assign CSS classes based on the node type and context. This is the most powerful approach, allowing distinctions like highlighting object keys (`object-key` class) differently from string values (`string` class).
            </li>
          </ul>
          <p>
          ...
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4 overflow-x-auto">
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">
              (Assumes CSS classes like <code>.json-object-key</code>, <code>.json-string</code>, etc. are defined elsewhere)
            </p>
          </div>
          <p>
         ...
         <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <strong>Incremental Parsing/Highlighting:</strong> Instead of re-parsing the entire document on every keystroke, only the changed portion and its affected structure are re-processed and re-highlighted. This requires a parser that can efficiently handle partial updates.
            </li>
            <li>
              <strong>Optimized Parsers:</strong> Using highly optimized parsing libraries, potentially written in lower-level languages like Rust or C++ and compiled to WebAssembly, for execution in web browsers or Node.js backends.
            </li>
            <li>
              <strong>Lazy Evaluation:</strong> For extremely large files, highlight only the visible portion and parts near the viewport, parsing and highlighting more as the user scrolls.
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <strong>Structural Highlighting:</strong> Use different colors or styles for different levels of nesting, matching brackets/braces, or separating commas. This helps visualize the tree structure.
            </li>
            <li>
              <strong>Semantic Highlighting (Advanced):</strong> In contexts where the JSON structure is known (e.g., a specific API response), you could potentially highlight specific keys differently (e.g., highlight the `"id"` key in blue, `"timestamp"` in green). This often requires additional schema information or contextual analysis and is less common in general-purpose JSON viewers.
            </li>
          </ul>
           <p>
