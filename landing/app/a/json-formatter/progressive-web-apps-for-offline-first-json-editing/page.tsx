    ...
    <article className="container mx-auto px-4 py-8 space-y-8">
      ...
      <section className="space-y-6">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300">
          Progressive Web Apps (PWAs) are a type of application delivered through the web, built using common web technologies like HTML, CSS, and JavaScript. They aim to combine the best of the web and mobile apps, providing features like offline access, push notifications, and installability on user devices, all from a single codebase accessible via a URL.
        </p>
        <p className="text-lg text-gray-700 dark:text-gray-300">
          The "progressive" aspect means they work for any user, regardless of browser choice or network connectivity, but they gain enhanced capabilities when used with modern browsers that support PWA features. Key components enabling these features include:
        </p>
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Service Workers:</strong> Scripts that run in the background, separate from the web page, intercepting network requests, caching assets, and enabling offline functionality and background sync.
          </li>
          <li>
            <strong>Web App Manifest:</strong> A JSON file that provides metadata about the web application (like name, icons, start URL, display mode) to enable installability and control its appearance when launched from the home screen.
          </li>
          <li>
            <strong>HTTPS:</strong> Essential for security and required for Service Workers to function, ensuring the connection between the user and the server is secure.
          </li>
        </ul>
      </section>
      ...
      <section className="space-y-6">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300">
          Offline-First is a design paradigm where the application is built assuming the user will spend most or all of their time offline. Data is stored locally on the device, and the user interface interacts directly with this local data. Synchronization with a remote server happens asynchronously in the background whenever a network connection is available.
        </p>
        <p className="text-lg text-gray-700 dark:text-gray-300">
        ...
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Reliability:</strong> The app remains usable regardless of network conditions (no network, slow network, flaky network).
          </li>
          <li>
            <strong>Performance:</strong> Reading and writing to local storage is significantly faster than making network requests.
          </li>
          <li>
            <strong>Responsiveness:</strong> UI updates can happen immediately based on local data, providing a smooth user experience.
          </li>
        </ul>
      </section>
      ...
      <section className="space-y-6">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300">
          Consider an application where users need to view and edit structured configuration files, notes, lists, or documents stored in JSON format. Examples include:
        </p>
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300">
          For these use cases, ensuring the user can access and modify the JSON data even when disconnected is crucial. An offline-first PWA is an ideal solution.
        </p>
      </section>
      ...
      <section className="space-y-6">
        ...
        <p className="text-lg text-gray-700 dark:text-gray-300">
          For storing JSON data client-side, <a href="https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">IndexedDB</a> is the most suitable Web API. It&apos;s a low-level API for client-side storage of significant amounts of structured data, including files/blobs. Unlike Local Storage, which is limited and synchronous, IndexedDB is asynchronous and works with object stores, making it perfect for storing multiple JSON documents or complex nested data.
        </p>
        <p className="text-lg text-gray-700 dark:text-gray-300">
          A typical IndexedDB setup involves opening a database, creating object stores (analogous to tables), and performing operations (add, put, get, delete) within transactions.
        </p>
...
`}
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            Note: IndexedDB is asynchronous and uses a request-based API. Libraries like <a href="https://github.com/jakearchibald/idb" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">idb</a> or <a href="https://dexie.org/" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">Dexie.js</a> can provide a more developer-friendly Promise or ORM-like interface.
          </p>
        </div>

        <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-300 flex items-center space-x-2">
          <CloudCog className="w-6 h-6" />{/* Changed from CloudSync to CloudCog */}
        ...
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Last Write Wins:</strong> The simplest approach. The most recent version (based on a timestamp) of a document overwrites older versions. This can lead to data loss if multiple users edit the same document offline simultaneously.
          </li>
          <li>
            <strong>Operational Transformation (OT):</strong> Records and applies individual editing operations (like "insert character 'A' at position 5"). This allows merging concurrent edits but is complex to implement, especially for structured data like JSON.
          </li>
          <li>
            <strong>Conflict Resolution:</strong> Detects conflicts (when the same data point is changed differently offline and on the server) and either:
          ...
          <li>
            <strong>Event Sourcing / CRDTs (Conflict-free Replicated Data Types):</strong> More advanced techniques designed specifically for distributed systems and offline-first scenarios to make merging easier and conflict-free.
          </li>
        </ul>
        <p className="text-lg text-gray-700 dark:text-gray-300">
...
`}
          ...
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            This conceptual code illustrates the push-then-pull flow. Real-world sync requires careful handling of timestamps, versions, deleted items, and robust conflict resolution based on the chosen strategy. Service Workers with the <a href="https://developer.mozilla.org/en-US/docs/Web/API/Background_Synchronization_API" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">Background Sync API</a> (though <a href="https://developer.mozilla.org/en-US/docs/Web/API/Periodic_Background_Sync_API" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">Periodic Background Sync</a> is experimental/limited) are ideal for performing these operations even when the app is closed.
          </p>
        </div>
        ...
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Cache App Shell:</strong> Store the core HTML, CSS, and JavaScript needed to run the application interface using the Cache API.
          </li>
          <li>
            <strong>Cache Data:</strong> Potentially cache frequently accessed JSON data or responses from sync endpoints.
          </li>
          <li>
            <strong>Intercept Requests:</strong> Serve cached responses when offline or on slow networks. For API requests related to data, it might check IndexedDB first, or queue requests for later sync.
          </li>
          <li>
            <strong>Handle Background Sync:</strong> Listen for the `sync` event (triggered by the Background Sync API when network is available) to initiate the synchronization process described above.
          </li>
        </ul>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4 overflow-x-auto text-sm">
          <h4 className="text-xl font-medium mb-3">Conceptual Service Worker Registration & Basic Fetch Interception:</h4>
          <pre className="bg-white p-3 rounded dark:bg-gray-900 text-gray-900 dark:text-gray-100">
...
`}
      ...
      <section className="space-y-6">
        ...
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Immediate Feedback:</strong> Actions like saving should update the UI instantly by writing to IndexedDB, without waiting for a server response.
          </li>
          <li>
            <strong>Indicate Sync Status:</strong> Show the user whether the app is online/offline and if there are pending changes or conflicts. A simple status indicator can help manage user expectations.
          </li>
          <li>
            <strong>Conflict Resolution UI:</strong> If using a conflict resolution strategy, provide a clear interface for the user to review conflicting versions and choose which changes to keep.
          </li>
          <li>
            <strong>Handle Initial Load:</strong> Ensure the app shell loads instantly from the Service Worker cache, providing a fast "first paint" even offline. Data is then loaded from IndexedDB.
          </li>
        </ul>
        <p className="text-lg text-gray-700 dark:text-gray-300">
...
`}
      ...
      <section className="space-y-6">
        ...
        <ul className="list-disc pl-8 space-y-3 text-lg text-gray-700 dark:text-gray-300">
          <li>
            <strong>Performance:</strong> JSON parsing and stringification can be expensive for large documents. Optimize your editor components and data handling.
          </li>
          <li>
            <strong>Background Sync API:</strong> While powerful, its availability and behavior can vary across browsers. Have fallbacks (e.g., manually triggering sync on online/offline events or app open).
          </li>
          <li>
            <strong>Error Handling:</strong> Implement robust error handling for IndexedDB operations, network requests, JSON parsing/validation, and sync conflicts. Provide clear feedback to the user.
          </li>
          <li>
            <strong>Security (HTTPS):</strong> Absolutely essential for Service Workers and securing data in transit.
          </li>
          <li>
            <strong>Data Structure Evolution:</strong> Plan how to handle changes to the JSON schema over time, both in your client-side IndexedDB and during sync with the server.
          </li>
          <li>
            <strong>User Experience:</strong> Make the offline capabilities clear. Don't let the user perform actions they think are synced when they aren't.
          </li>
        </ul>
      </section>
