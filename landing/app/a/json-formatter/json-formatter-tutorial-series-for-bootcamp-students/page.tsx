...
export default function JsonFormatterTutorial() {
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>A collection of name/value pairs (like an object in JavaScript, a dictionary in Python, a hash table in many languages).</li>
          <li>An ordered list of values (like an array in JavaScript, a list in Python).</li>
        </ul>
        <p>
          At its core, JSON represents data using key-value pairs within objects (<code>&#x7b; &#x7d;</code>) and ordered lists within arrays (<code>[ ]</code>), combined with primitive data types like strings, numbers, booleans (<code>true</code>, <code>false</code>), and <code>null</code>.
        </p>
  ...
  "address": {
    ...
    "city": "Anytown"
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Improved Readability:</strong> Quickly understand the structure and content of JSON data.</li>
          <li><strong>Easier Debugging:</strong> Pinpoint missing commas, misplaced brackets, or other syntax errors visually.</li>
          <li><strong>Smoother Collaboration:</strong> Share cleanly formatted data with teammates.</li>
          <li><strong>Syntax Validation:</strong> Many formatters also check if your JSON is valid.</li>
        </ul>

        <h2 className="text-2xl font-semibold mt-8">
          <AlertTriangle className="inline-block mr-2 text-yellow-500" /> Common JSON Formatting Issues (and How Formatters Help)
        </h2>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li><strong>Trailing Commas:</strong> Including a comma after the last item in an object or array (this is invalid in standard JSON, though some parsers might allow it).</li>
          <li><strong>Incorrect Quotes:</strong> Using single quotes (`'`) instead of double quotes (`"`) for keys and string values. Keys *must* be double-quoted strings.</li>
          <li><strong>Unescaped Characters:</strong> Not escaping special characters within strings (like backslashes `\` or double quotes `"`).</li>
          <li><strong>Comments:</strong> Adding comments (`//` or `/* */`) directly in the JSON data (JSON does not support comments).</li>
        </ul>
        <p>
        ...
        <ol className="list-decimal pl-6 space-y-2 my-4">
          <li><strong>Parsing:</strong> It reads the raw JSON text and converts it into an in-memory representation (like JavaScript objects, arrays, strings, numbers, booleans, null). During this step, it validates the syntax. If there's an error, the parsing fails.</li>
          <li><strong>Pretty-Printing (Serialization):</strong> It takes the valid in-memory representation and converts it back into a string format with added whitespace (indentation and newlines) according to specified rules (like using 2 spaces, 4 spaces, or tabs for indentation).</li>
        </ol>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Pros:</strong> No installation needed, accessible from anywhere, often include validation and tree views.</li>
          <li><strong>Cons:</strong> Requires pasting potentially sensitive data into a third-party website (be cautious!), depends on internet connection.</li>
        </ul>
        <p>
        ...
        <p className="flex items-center text-blue-600 dark:text-blue-400 italic">
          <ExternalLink className="inline-block mr-2" size={18} /> (Example: A quick search for "online json formatter" will yield many results).
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Pros:</strong> Integrated into your workflow, formats files directly, often configurable (indent size, etc.).</li>
          <li><strong>Cons:</strong> Requires editor installation/configuration.</li>
        </ul>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Pros:</strong> Great for scripting, processing large files, works well with pipes (`|`).</li>
          <li><strong>Cons:</strong> Requires using the terminal, syntax can be less intuitive for beginners.</li>
        </ul>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Pros:</strong> Control formatting within your application, useful before saving or sending data.</li>
          <li><strong>Cons:</strong> Requires writing code, might not be needed for simple viewing/debugging.</li>
        </ul>
        <p>
...
*/`}
        ...
        <p>
          The third argument of <code>JSON.stringify()</code> controls indentation. Use a number (like 2 or 4) for spaces or a string (like <code>"\\t"</code>) for tabs. The second argument (<code>null</code> here) is for a "replacer" function or array, which you can use to filter or transform values during serialization (advanced topic!).
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li><strong>Format Often:</strong> Get into the habit of formatting JSON whenever you're inspecting API responses or editing configuration files.</li>
          <li><strong>Use Your IDE:</strong> Learn the shortcut for formatting in your code editor (e.g., <code>Shift+Alt+F</code> in VS Code by default on Windows/Linux, <code>Shift+Option+F</code> on macOS).</li>
          <li><strong>Validate First:</strong> If a formatter tool gives you an error, fix the syntax errors before trying to format again. The validation step is crucial.</li>
          <li><strong>Understand the Structure:</strong> Formatting helps, but truly understanding JSON means knowing the difference between objects (<code>&#x7b; &#x7d;</code>) and arrays (<code>[ ]</code>) and how they nest.</li>
          <li><strong>Be Mindful of Sensitive Data:</strong> Avoid pasting sensitive JSON into public online formatters.</li>
        </ul>
