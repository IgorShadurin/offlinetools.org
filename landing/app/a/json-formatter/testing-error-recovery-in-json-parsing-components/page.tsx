...
export const metadata: Metadata = {
  ...
  description:
    "Learn about strategies and testing methodologies for error recovery in JSON parsing components.",
};
    ...
    <>
      ...
      <div className="space-y-6">
        <p>
          ...
          Parsing JSON, the process of converting a JSON string into a usable data structure (like a JavaScript object or array), is a fundamental operation in many applications.
          While standard libraries provide robust parsers, developers sometimes build custom JSON parsing components, particularly in scenarios requiring specific optimizations, streaming capabilities, or when working with constrained environments.
        </p>
        <p>
          A critical, yet often overlooked, aspect of building any parser, including a JSON parser, is <strong>error handling and recovery</strong>. A parser needs to gracefully handle malformed input, report errors accurately, and ideally, attempt to recover from errors to find more errors or provide a partially parsed result where possible.
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Manual Editing:</strong> Users might manually edit configuration files or data structures in JSON format and introduce typos.
          </li>
          <li>
            <strong>Faulty Data Sources:</strong> External systems or APIs might occasionally send malformed JSON due to bugs or network issues.
          </li>
          <li>
            <strong>Streaming/Partial Data:</strong> In streaming scenarios, data might be truncated or corrupted mid-stream.
          </li>
          <li>
            <strong>Developer Tooling:</strong> Parsers in linters, formatters, or IDEs benefit greatly from error recovery to provide multiple error messages instead of stopping on the first syntax issue.
          </li>
        </ul>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            ...
            <ul className="list-circle pl-6 space-y-1 my-2">
              ...
              <li>Unclosed brackets <code>[</code> or braces <code>&#x7b;</code>.</li>
        ...
        <p>
          This is the simplest strategy. When an error is detected, the parser discards input tokens until it finds a "synchronization token" – a token that is likely to appear after a grammatical construct. For JSON, potential synchronization tokens might be <code>,</code>, <code>]</code>, or <code>&#x7d;</code>.
        </p>
        <p>
          Example: Parsing <code>[1, 2 invalid 3, 4]</code>.
          The parser reads <code>[</code>, <code>1</code>, <code>,</code>, <code>2</code>, <code>invalid</code>. It sees <code>invalid</code>, which is unexpected after <code>2</code>.
          In panic mode, it discards <code>invalid</code> and looks at the next token, <code>3</code>. Still unexpected (missing comma or end of array). Discard <code>3</code>. Look at <code>,</code>. Ah, a comma! This could be a synchronization token. It then expects a value after the comma (which is <code>4</code>).
          It might report an error about "unexpected token 'invalid'" and possibly "unexpected token '3'". It might successfully parse <code>[1, 2, 4]</code>, but the recovery skipped <code>3</code> entirely, which might not be desired.
        </p>
        <p>
...
try {
  ...
  // or at the end of the input.
        ...
        <p>
          ...
          The parser reads <code>&#x7b;</code>, <code>"name"</code>, <code>:</code>, <code>"Alice"</code>. It then expects a <code>,</code> or <code>&#x7d;</code>. It sees <code>"age"</code>.
          A phrase-level recovery might diagnose "missing comma before 'age'" and conceptually insert a comma, then continue parsing <code>, "age": 30</code> as a new property.
        </p>
        <p>
  ...
  // If we reach here and currentToken is BraceClose, the loop condition handles it.
        ...
        <p>
          ...
          <code>ErrorObjectBody ::= String ":" Value // Handles a key-value pair where a comma was expected before it</code>
        </p>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Error Reporting:</strong> Are errors reported accurately (correct type of error, location - line/column)?
          </li>
          <li>
            <strong>Recovery Success:</strong> Does the parser successfully continue parsing after the error?
          </li>
          <li>
            <strong>Subsequent Error Detection:</strong> Can the parser find multiple errors in a single malformed input?
          </li>
          <li>
            <strong>Output After Recovery:</strong> If the parser produces a partial result, is it meaningful or at least predictable? (This is less common for typical JSON parsers but relevant for linters/formatters).
          </li>
          <li>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Single Errors:</strong> Introduce one specific type of error at different locations (start, middle, end of arrays, objects, strings).
            <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
{"data": true} extra`}
          ...
          <li>
            <strong>Multiple Errors:</strong> Combine several single errors in one input string. This tests if the recovery from the first error allows the parser to encounter the second.
            <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <li>
            <strong>Edge Cases:</strong> Test errors involving empty structures <code>&#x7b;&#x7d;</code>, <code>[]</code>, deeply nested structures, very long strings, large numbers, etc.
          </li>
          <li>
            <strong>Invalid Characters/Tokens:</strong> Feed the parser input with completely foreign characters or sequences that don't belong in JSON.
          </li>
        </ul>
