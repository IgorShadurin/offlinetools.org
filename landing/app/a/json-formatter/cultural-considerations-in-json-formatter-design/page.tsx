    ...
    <>
      ...
      <div className="space-y-6 text-lg leading-relaxed">
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Decimal Separator:</strong> In many English-speaking countries, a dot (`.`) is used (e.g., <code>123.45</code>). In many European and South American countries, a comma (`,`) is used (e.g., <code>123,45</code>).
          </li>
          <li>
            <strong>Thousands Separator:</strong> The character and grouping used for thousands separators also vary. Common separators include comma (`,`), dot (`.`), and space (` `). Grouping can be by three digits (<code>1,000,000</code>), but sometimes by four or other numbers (e.g., in India, <code>10,00,000</code>).
          </li>
          <li>
            <strong>Currency:</strong> If the number represents currency, the currency symbol position and separator conventions become even more critical.
          </li>
        </ul>
        <p>
        ...
        <p>
          JSON does not have a built-in date/time type. Dates and times are typically represented as strings, often in ISO 8601 format (e.g., <code>&quot;2023-10-27T10:00:00Z&quot;</code>). While ISO 8601 is excellent for machine readability and interchange, its display format for humans is not universally preferred.
        </p>
        <p>
          Cultural date formats include:
        </p>
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>Month/Day/Year (<code>MM/DD/YYYY</code>, e.g., <code>10/27/2023</code> in the US)</li>
          <li>Day/Month/Year (<code>DD/MM/YYYY</code>, e.g., <code>27/10/2023</code> in the UK and many other places)</li>
          <li>Year/Month/Day (<code>YYYY/MM/DD</code>, e.g., <code>2023/10/27</code>, common in East Asia and often preferred for sorting)</li>
        </ul>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Character Sets and Fonts:</strong> While UTF-8 is standard for JSON encoding, ensure the display environment (the formatter UI) correctly renders characters from various scripts (Latin, Cyrillic, Arabic, CJK, etc.) and symbols, including emojis.
          </li>
          <li>
            <strong>Right-to-Left (RTL) Text:</strong> Languages like Arabic and Hebrew are read from right to left. A simple formatter might not correctly handle the layout of strings containing RTL text mixed with LTR elements or punctuation.
          </li>
          <li>
            <strong>Collation and Sorting:</strong> While not strictly a formatting issue, if a formatter offers sorting of keys or array elements, the collation rules (how strings are alphabetically ordered) are highly locale-dependent (e.g., `ä` vs `a`, handling of accents, case sensitivity).
          </li>
        </ul>

        <h3 className="text-xl font-semibold mt-6">Handling Null and Empty Values</h3>
        <p>
          JSON has a distinct <code>null</code> type. It also allows empty strings (<code>&quot;&quot;</code>), empty arrays (<code>[]</code>), and empty objects (<code>&#x7b;&#x7d;</code>).
        </p>
        <p>
          How these are presented can matter:
        </p>
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>Should <code>null</code> be displayed as &quot;null&quot;, &quot;N/A&quot;, &quot;-&quot;, or perhaps left blank? The choice might depend on context and cultural expectations for representing missing information.</li>
          <li>Should empty strings/arrays/objects be visually distinct from properties that are explicitly <code>null</code>?</li>
        </ul>

        <h3 className="text-xl font-semibold mt-6">Key Naming Conventions</h3>
        <p>
          While not strictly a cultural issue, key names (like <code>firstName</code>, <code>last_name</code>) often follow programming language conventions (camelCase, snake_case, PascalCase) which can sometimes align with regional technical preferences or historical practices within certain tech communities. A formatter doesn't usually change key names, but presenting them clearly is key.
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Language:</strong> Error messages (e.g., &quot;Invalid JSON&quot;, &quot;Unexpected token&quot;) should be localizable.
          </li>
          <li>
            <strong>Clarity and Tone:</strong> The phrasing of messages and UI labels should be clear, concise, and culturally appropriate.
          </li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Localization (L10n) and Internationalization (I18n):</strong> Implement standard practices for software internationalization, separating translatable strings and using locale-aware formatting APIs provided by programming languages or libraries for numbers and dates.
          </li>
          <li>
            <strong>Configuration Options:</strong> Allow users to select their preferred locale or customize formatting options (e.g., choose decimal separator, date format).
          </li>
          <li>
            <strong>Adhere to Standards:</strong> For data interchange, stick to unambiguous standards like ISO 8601 for dates. Cultural formatting is primarily for *display*.
          </li>
          <li>
            <strong>Test with Diverse Data:</strong> Use JSON containing text from various languages, different number formats (if represented as strings), and test rendering on systems configured with different regional settings.
          </li>
          <li>
            <strong>Consider Context:</strong> If the formatter knows the *type* of data (e.g., it's currency, a date), it can apply appropriate formatting rules.
          </li>
        </ul>
