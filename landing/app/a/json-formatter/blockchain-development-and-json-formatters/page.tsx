      ...
      <div className="space-y-8">
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-3">
            <li>
              <span className="font-medium">Transaction Payloads:</span> While the core transaction data might be serialized in binary formats for efficiency on some chains, the "payload" or "memo" field often accepts arbitrary data. JSON is frequently used here to attach structured metadata, messages, or application-specific instructions to a transaction.
                  ...
                  <code>{`// Example JSON-RPC call to a smart contract function
{
  ...
  "id": 1
            ...
            <li>
              <span className="font-medium">Wallets and dApps:</span> Frontend applications (dApps) and wallets heavily rely on JSON to represent account information, transaction details, network configurations, and data fetched from blockchain nodes. User interfaces are built by parsing JSON data received from the blockchain or APIs.
            </li>
            <li>
              <span className="font-medium">APIs and Oracles:</span> External services (like centralized exchanges, data providers, oracles) that interact with blockchains often expose JSON APIs. Oracles, which bring real-world data onto the blockchain, parse external data sources (frequently JSON) and format it for on-chain consumption.
            </li>
            <li>
              <span className="font-medium">Configuration and Metadata:</span> Many blockchain tools, network configurations, token metadata (like ERC-721 metadata URIs), and decentralized identity documents (like DIDs) are stored or defined using JSON.
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-3 mb-4">
            <li>
              <span className="font-medium">Parsing (Deserialization):</span> Converting a JSON string into a native programming language data structure (like a JavaScript object or array, a Python dictionary, etc.).
            </li>
            <li>
              <span className="font-medium">Stringifying (Serialization):</span> Converting a native programming language data structure into a JSON string.
            </li>
          </ul>
          <p>
...
`}
          ...
          <ul className="list-disc pl-6 space-y-3">
            <li>
              <span className="font-medium">Pretty-Printing:</span> Adding whitespace and indentation to make JSON human-readable. Useful for logging, debugging, and displaying data in user interfaces.
            </li>
            <li>
              <span className="font-medium">Minification:</span> Removing all unnecessary whitespace to reduce the size of the JSON string. Useful for reducing data transfer size over networks.
            </li>
            <li>
              <span className="font-medium">Validation:</span> Checking if a string is valid JSON or if a JSON object conforms to a specific schema (like JSON Schema). Crucial for ensuring data integrity.
            </li>
            <li>
              <span className="font-medium">Canonicalization:</span> Converting a JSON object into a deterministic string representation (e.g., by sorting keys alphabetically and removing whitespace). This is particularly important in blockchain.
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <p className="mb-4">
            Standard JSON allows for variations in formatting (like whitespace) and object key order. For example, <code>{`{"a": 1, "b": 2}`}</code> and <code>{`{"b": 2, "a": 1}`}</code> represent the same logical object in JSON but are different strings.
          </p>
          <p>
          ...
          <p className="mb-4">
            <span className="font-medium">Canonical JSON</span> addresses this by specifying a strict format for serialization:
          </p>
