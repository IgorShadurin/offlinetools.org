    ...
    <>
      ...
      <div className="space-y-6">
        <p>
          ...
          interaction can limit accessibility and efficiency. Implementing robust keyboard navigation is crucial
          for users who prefer keyboard control, have motor impairments, or use screen readers. This article
          explores the principles and techniques for adding keyboard navigation to your JSON tree view component.
        </p>

        <h2 className="text-2xl font-semibold mt-8">Why Keyboard Navigation Matters</h2>
        <p>
          Adding keyboard support to interactive components like tree views isn&apos;t just a feature; it&apos;s
          a fundamental aspect of good user experience and accessibility.
        </p>
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          ...
          <ul className="list-disc pl-6 space-y-2 mt-2">
            <li>
              <span className="font-medium">Accessibility:</span> Enables users with motor disabilities or those
              using screen readers to fully interact with the tree view.
            </li>
            <li>
              <span className="font-medium">Efficiency:</span> Allows power users to quickly navigate and
              manipulate the tree structure without lifting their hands from the keyboard.
            </li>
            <li>
              <span className="font-medium">Compliance:</span> Helps meet accessibility standards like WCAG (Web
        ...
        <p>
          Implementing keyboard navigation for a tree view primarily involves managing focus and handling key
          events.
        </p>

        <h3 className="text-xl font-semibold mt-6">1. Focus Management</h3>
        <p>
          Only one item in the tree view should be "focused" at any given time for keyboard interaction. This
          focused item receives keyboard events. The focus should be visually indicated, typically with an outline.
        </p>
        ...
        <p>
          You need to listen for keyboard events on the tree view container or individual tree nodes. The most
          relevant events are `onKeyDown` or `onKeyPress`.
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 mt-2">
          ...
          <li>
            <span className="font-mono">ArrowRight</span>: If focused on a collapsed node, expand it. If focused
            on an expanded node, move focus to its first child. If focused on a leaf node, do nothing or move to
            the next sibling (depending on implementation).
          </li>
          <li>
            <span className="font-mono">ArrowLeft</span>: If focused on an expanded node, collapse it. If focused
            on a collapsed node or leaf node, move focus to its parent.
          </li>
          <li>
            <span className="font-mono">Enter</span> or <span className="font-mono">Space</span>: Toggle the
            expanded state of a node or activate/select a leaf node.
          </li>
          <li>
        ...
        <p>
          Let&apos;s look at a conceptual outline of how you might implement this in a React/Next.js component
          using TSX.
        </p>
...
`}
          ...
          <p className="mt-2 text-sm">
            ...
            <br />
            4. `JsonTreeView` is the main component. It holds the parsed `treeData` and the `focusedNodePath`
            state.
            <br />
            5. The `onKeyDown` handler in `JsonTreeView` is the core of navigation. It checks `event.key` and
            calculates the `nextFocusedPath` based on the current `focusedNodePath`, the tree structure, and the
            expansion state of nodes (this requires a helper function like `getVisibleNodes`).
            <br />
            6. `setFocusedNodePath` updates the state, which triggers re-renders. The `isFocused` prop is passed
            down, causing the relevant `JsonTreeNode` to receive focus programmatically.
            <br />
            7. ARIA attributes like `role=&quot;tree&quot;`, `role=&quot;treeitem&quot;`,
            `aria-expanded`, and `aria-activedescendant` are crucial for accessibility. `aria-activedescendant`
            on the container (`ul`) tells screen readers which item within the tree is currently focused, even if
            native browser focus (`tabIndex=0`) is only on the container or the actively selected item.
          </p>
        </div>

        <h2 className="text-2xl font-semibold mt-8">Key Implementation Challenges</h2>
        <p>
          While the concept is straightforward, implementing robust keyboard navigation in a dynamic tree view
          involves handling complexities:
        </p>

        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <ul className="list-disc pl-6 space-y-3">
            <li>
              <span className="font-medium">Maintaining Visible Node List:</span> The order of nodes when
              navigating `ArrowDown` or `ArrowUp` depends on which parent nodes are expanded. You need an
              efficient way to determine the sequence of currently visible nodes.
            </li>
            <li>
              <span className="font-medium">Complex Navigation Logic:</span> The logic for `ArrowLeft` (finding
              the parent or collapsing) and `ArrowRight` (expanding or finding the first child) can be intricate,
              especially in deeply nested structures.
            </li>
            <li>
              <span className="font-medium">Scrolling Into View:</span> When focus moves to a node not currently
              visible in the viewport, you need to programmatically scroll the container to make it visible.
            </li>
            <li>
              <span className="font-medium">Performance:</span> For very large JSON structures, recalculating
              visible nodes or re-rendering large parts of the tree on every key press needs to be optimized.
            </li>
            <li>
              ...
              `aria-selected`, `aria-activedescendant`) is vital for screen reader users.
            </li>
          </ul>
        </div>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            Use a single state variable (e.g., `focusedNodePath`) at the highest relevant level to manage which
            node is currently focused.
          </li>
          <li>
            Implement `onKeyDown` on the main container (`ul` or a wrapper div) and manage focus programmatically
            on the individual `li` elements.
          </li>
          <li>
            Set `tabIndex=&quot;0&quot;` only on the currently focused node, and `tabIndex=&quot;-1&quot;` on all
            other interactive nodes within the tree. Alternatively, use `aria-activedescendant` on the container
            and manage focus virtually without relying on native `tabIndex` on every item. The ARIA method is
            generally preferred for tree views.
          </li>
          <li>
        ...
        <p>
          Adding keyboard navigation to a JSON tree view significantly enhances its usability and makes it
          accessible to a wider range of users. While the implementation requires careful handling of focus and
          complex navigation logic, the benefits in terms of user experience and compliance are substantial. By
          focusing on core concepts like focus management, event handling, and proper ARIA usage, you can create a
          powerful and accessible tree view component. Remember to test your implementation rigorously to ensure it
          behaves as expected for all users.
        </p>
      </div>
    </>
  );
