  ...
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <p>
          <strong>Zapier</strong> is a powerful automation tool that connects thousands of web applications. It allows you to create automated workflows (called "Zaps") where an event in one app (the Trigger) kicks off an action or series of actions in other apps. Integrating a JSON formatter service into your Zapier workflow allows you to automatically clean up JSON data as it moves from one application to another.
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Readability & Debugging:</strong> When JSON is properly indented, it's much easier for humans to read and understand its structure, which is crucial when inspecting data flowing through your Zapier steps or logging it for debugging.
          </li>
          <li>
            <strong>Consistency:</strong> Ensure that downstream applications or systems consistently receive JSON in a predictable, formatted structure, even if the source provides it inconsistently.
          </li>
          <li>
            <strong>Compatibility:</strong> While most parsers are tolerant of whitespace, some legacy systems or specific tools might process formatted JSON more reliably.
          </li>
          <li>
            <strong>Logging & Archiving:</strong> Storing formatted JSON in logs, databases, or spreadsheets makes future analysis and troubleshooting much simpler.
          </li>
        </ul>
        ...
        <p>
          ...
          <ol className="list-decimal pl-6 space-y-2 my-4">
            <li>Triggers when new data arrives from a webhook (App A), which provides data as a single-line JSON string.</li>
            <li>Needs to store this JSON data in a readable format in a Google Sheet or send it to a logging service (App B).</li>
            <li>Might use some data from within the JSON in another step (App C), and extracting values from unformatted JSON within Zapier's interface can be tricky.</li>
          </ol>
        ...
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <div className="flex items-center justify-center space-x-4 text-lg font-mono">
            <span className="flex items-center space-x-1"><Database size={20} /> App A (Trigger)</span>
            <ArrowRight size={20} />
            <span className="flex items-center space-x-1"><Settings size={20} /> JSON Formatter Service (Action)</span>
            <ArrowRight size={20} />
            <span className="flex items-center space-x-1"><Link size={20} /> App B (Action)</span>
          </div>
...
&#x7d;;`}
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-3">
            In this example, <code>inputData.json_string</code> is where Zapier would pass the JSON string from a previous step. The code uses the built-in <code>JSON.parse()</code> and <code>JSON.stringify()</code> methods available in the Node.js runtime to parse and then re-serialize the JSON with indentation (<code>null, 2</code> specifies no replacer function and 2 spaces for indentation). The result is made available to subsequent steps in the <code>output</code> object. Basic error handling is included to catch invalid JSON.
          </p>
        </div>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>
            <strong>Improved Data Visibility:</strong> Makes the data passing through your Zaps easier to inspect in Zapier's task history.
          </li>
          <li>
            <strong>Streamlined Integrations:</strong> Ensures data is in the desired format for downstream applications.
          </li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          ...
          <li>
            <strong>Code Step Limits:</strong> The Code step has execution time and memory limits. Very large or deeply nested JSON might hit these limits.
          </li>
          <li>
            <strong>Error Handling:</strong> Implement robust error handling in your Code step or API call to gracefully manage invalid JSON input.
          </li>
          <li>
            <strong>Security:</strong> Be mindful of sensitive data when sending JSON to external formatting APIs. Using the built-in Code step keeps data within Zapier's processing environment.
          </li>
        </ul>
        ...
        <ol className="list-decimal pl-6 space-y-3 my-4">
          <li>
            <strong>Trigger:</strong> Set up a "Webhooks by Zapier" trigger to "Catch Hook". This URL receives JSON data from another service.
          </li>
          <li>
            <strong>Action 1 (Code by Zapier):</strong> Add a "Code by Zapier" action, selecting "Run Javascript".
            <ul className="list-disc pl-6 space-y-1 my-2">
              <li>Under "Edit Template", define an input variable, e.g., <code>json_string</code>, and map it to the raw JSON output from your Webhook trigger step. Zapier provides fields like "Catch Hook" -&gt; "Raw Body" or similar depending on the webhook data structure.</li>
          ...
          <li>
            ...
            <ul className="list-disc pl-6 space-y-1 my-2">
              ...
              <li>For the "File" field, select the output from your "Code by Zapier" step. Specifically, map the <code>formatted_json</code> output variable.</li>
