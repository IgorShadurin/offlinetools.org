    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <strong>Language Definition:</strong> You need to define a specific <code>com.intellij.lang.Language</code> instance for JSON (or your custom JSON-like language). While a standard JSON language is built-in, for a custom formatter, you might need to override aspects or define a new language ID if it's a variation.
          </li>
          <li>
            <strong>Parser Definition:</strong> A <code>com.intellij.lang.ParserDefinition</code> tells the IDE how to parse your language's text into a PSI tree. This is crucial because the formatter operates on this tree structure.
          </li>
          <li>
            <strong>Formatter Provider:</strong> A <code>com.intellij.psi.codeStyle.LanguageInjector</code> is used to inject a formatter into specific contexts, but for a primary language formatter, you mostly rely on standard extensions. The core formatting logic is provided via a <code>com.intellij.psi.codeStyle.FormattingModelBuilder</code>.
          </li>
          <li>
            <strong>Formatting Model Builder:</strong> The <code>FormattingModelBuilder</code> creates a <code>com.intellij.psi.codeStyle.FormattingModel</code> for a given PSI element. This model contains the structure (using "Blocks") and rules for formatting.
          </li>
          <li>
            <strong>Block Implementation:</strong> This is the heart of the formatter. You'll create classes implementing <code>com.intellij.formatting.Block</code> (often by extending <code>com.intellij.formatting.AbstractBlock</code> or similar base classes provided by the platform). Each Block corresponds to a node or a range of nodes in the PSI tree and is responsible for:
          ...
          <li>
            <strong>Code Style Settings:</strong> To make your formatter configurable via the IDE's settings dialog, you'll implement classes extending <code>com.intellij.psi.codeStyle.CodeStyleSettings</code> and integrate a settings panel using Swing or Kotlin UI DSL.
          </li>
        </ul>
        ...
        <p>
          Create classes for your language (e.g., <code>JsonishLanguage</code>) and its parser definition (e.g., <code>JsonishParserDefinition</code>). The parser definition links your lexer and parser implementations to the language.
        </p>
...
&lt;/extensions&gt;`}
        ...
        <p>
          Create a class (e.g., <code>JsonishFormattingModelBuilder</code>) that implements <code>FormattingModelBuilder</code>. Its main method, <code>createModel</code>, will take a PSI element and return a <code>FormattingModel</code>. This model is built around your custom <code>Block</code> implementation.
        </p>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <h4 className="text-lg font-medium">Example plugin.xml snippet (conceptual):</h4>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto">
...
&lt;/extensions&gt;`}
        ...
        <p>
          This is the most complex part. You'll likely create a class like <code>JsonishBlock</code> (extending <code>AbstractBlock</code> or similar).
        </p>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            The <code>buildChildren()</code> method recursively creates child <code>Block</code> objects for the children of the current PSI element.
          </li>
          <li>
            The <code>getIndent()</code> method returns the indentation for the current block based on its type (e.g., increase indent inside objects and arrays).
          </li>
          <li>
            The <code>getSpacing()</code> method defines spaces between child elements (e.g., space after colon, space after comma). This uses the <code>com.intellij.formatting.SpacingBuilder</code>.
          </li>
          <li>
            The <code>getWrap()</code> method defines wrapping rules (e.g., wrap array elements if they exceed line length).
          </li>
          <li>
        ...
        <p>
          You'll need to identify the specific PSI element types produced by your parser (e.g., object, array, property, key, value, comma, colon) and write logic in your <code>JsonishBlock</code> to handle the formatting rules for each type.
        </p>

        <h3 className="text-xl font-semibold mt-6">5. Add Code Style Settings (Optional but Recommended)</h3>
        <p>
          Implement classes to manage settings and create a UI panel. This allows users to customize indentation size, spacing options, etc., for your formatter. This involves working with <code>com.intellij.psi.codeStyle.CodeStyleSettingsProvider</code> and <code>com.intellij.psi.codeStyle.CustomCodeStyleSettings</code>.
        </p>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <h4 className="text-lg font-medium">Example plugin.xml snippet (conceptual):</h4>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto">
...
&lt;/extensions&gt;`}
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li><code>Indent.getNoneIndent()</code>: No indentation (for elements at the top level or within containers that don't indent their children).</li>
          <li><code>Indent.getNormalIndent()</code>: Standard indentation level (for children of objects/arrays).</li>
          <li><code>Indent.getSmartIndent(type)</code>: Context-aware indentation.</li>
        </ul>
        <p>
          You'll typically define that children of object (&#x7b;...&#x7d;) and array ([&#x7b;...&#x7d;]) PSI elements get <code>NormalIndent</code> relative to their parent block.
        </p>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
return spacingBuilder.getSpacing(getParent(), psiElement, child.getNode());`}
        ...
        <p>
          Use <code>com.intellij.formatting.Wrap.createWrap()</code> and <code>com.intellij.formatting.Alignment.createAlignment()</code>. These are used within your Block's <code>buildChildren()</code> method to associate wrapping or alignment rules with specific child blocks.
        </p>
        <ul className="list-disc pl-6 space-y-2">
          <li>
            Wrapping: You can enforce wrapping (<code>Wrap.createWrap(WrapType.ALWAYS, true)</code>), wrap if needed (<code>WrapType.CHOP_DOWN_IF_NECCESSARY</code>), etc. Useful for putting object properties or array elements on new lines.
          </li>
          <li>
...
if (childPsi.getNode().getElementType() == JsonishElementTypes.PROPERTY_VALUE) {
  ...
  // ... handle other types ...
        ...
        <ol className="list-decimal pl-6 space-y-2">
          ...
          <li>Create a root <code>JsonishBlock</code> for this element.</li>
          <li>Initialize a <code>SpacingBuilder</code> with rules based on the current code style settings.</li>
          <li>Pass the <code>SpacingBuilder</code> and settings down through the recursive creation of child blocks in the <code>buildChildren()</code> method of your <code>JsonishBlock</code>.</li>
          <li>The <code>getSpacing()</code> method in each block uses the <code>SpacingBuilder</code> to determine spacing between its own children.</li>
          <li>The <code>getIndent()</code>, <code>getWrap()</code>, and <code>getAlignment()</code> methods define rules applied *to* the current block relative to its parent, or applied *to* its children.</li>
        </ol>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li><strong>PSI Tree Understanding:</strong> The biggest hurdle is understanding the structure of the PSI tree generated by your parser (or the standard JSON parser). Use the "PsiViewer" plugin in the IDE Dev instance to inspect the tree for example JSON code.</li>
          <li><strong>SpacingBuilder Complexity:</strong> Configuring the <code>SpacingBuilder</code> for all token types and contexts can be tricky. Start simple and add rules incrementally.</li>
          <li><strong>Debugging:</strong> Debugging formatter code can be challenging. Use logging and the IDE's debugging tools extensively while running a Dev instance of the IDE with your plugin.</li>
          <li><strong>Performance:</strong> Formatting is a frequent operation. Ensure your block creation and rule application logic is efficient to avoid performance bottlenecks. Avoid excessive PSI traversals within formatting methods.</li>
          <li><strong>Testing:</strong> Write tests for your formatter. The platform provides testing utilities for formatting.</li>
        </ul>
        ...
        <p>
          Building a custom formatter plugin for JetBrains IDEs, even for a seemingly simple format like JSON, is a non-trivial task that requires understanding the IntelliJ Platform's architecture, particularly its PSI and formatting APIs. However, it offers immense power to create highly tailored code style experiences. By defining your language elements, building a robust PSI tree, and implementing the <code>Block</code> structure with precise rules for indentation, spacing, wrapping, and alignment, you can integrate a professional-grade formatter directly into the developer's workflow within their preferred IDE. While this article provides a high-level overview, the actual implementation details reside within the IntelliJ Platform SDK documentation and require coding in Java or Kotlin.
        </p>
      </div>
    </>
  );
