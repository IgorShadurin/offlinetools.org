...
import {
  ...
  Info,
  Lightbulb,
} from "lucide-react"; // Import only allowed icons
...
export default function LearnJsonFormatterErrorsArticle() {
  return (
    <>
      ...
      <div className="space-y-6">
        ...
        <p>
          JSON parsers are strict. Unlike some other data formats or programming languages that might tolerate minor inconsistencies, a JSON parser will typically halt and report an error upon encountering anything that doesn't strictly adhere to the JSON specification (<a href="https://www.json.org/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">json.org</a>).
        </p>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li>Incorrect capitalization of literals (<code>true</code>, <code>false</code>, <code>null</code>).</li>
        ...
        <ol className="list-decimal pl-6 space-y-2">
          <li>
            <strong>Pretty-Printing:</strong> It takes a compact or poorly indented JSON string and outputs a human-readable, indented version. This reveals the structure of the data, making it easier to visually inspect.
          </li>
          <li>
            <strong>Validation:</strong> Crucially, before formatting, a good formatter attempts to parse the JSON. If the parsing fails, it will typically report the location and nature of the syntax error.
          </li>
        </ol>
        <p>
  ...
  "age": 30 // <-- Missing comma here
          ...
          <p className="text-sm">
            The formatter tells you it expected a comma (<code>,</code>) or the closing brace (<code>&#x7d;</code>) after parsing the <code>"name"</code> property but found the start of the next property (<code>"age"</code>) instead.
            <br />
  ...
  "item2": 2 // Removed comma
        ...
        <h3 className="text-xl font-semibold mt-6 flex items-center gap-2">
          <CircleAlert className="w-5 h-5 text-red-500" /> Error: Expected string or '&#x7d;' (or similar related to keys)
        </h3>
        <p>
  ...
  "city": "London" // Replaced single quotes with double quotes
        ...
        <h3 className="text-xl font-semibold mt-6 flex items-center gap-2">
          <CircleAlert className="w-5 h-5 text-red-500" /> Error: Expected string or primitive value (or similar related to values)
        </h3>
        <p>
  ...
  "description": "This is a "quote"." // <-- Unescaped quote inside string
          ...
          <h4 className="text-lg font-medium mt-4 mb-2">Explanation & Fix:</h4>
          <p className="text-sm">
            A double quote character (<code>"</code>) inside a string value must be escaped with a backslash (<code>\</code>). Other characters requiring escaping include <code>\</code>, <code>/</code>, backspace (<code>\b</code>), form feed (<code>\f</code>), newline (<code>\n</code>), carriage return (<code>\r</code>), and tab (<code>\t</code>). Unicode characters can be escaped using <code>\uXXXX</code>.
            <br />
  ...
  "description": "This is a \\"quote\\"." // Escaped inner quotes
        ...
        <p>
          This often happens when boolean (<code>true</code>, <code>false</code>) or null (<code>null</code>) values are incorrectly capitalized.
        </p>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
  ...
  "isEnabled": True // <-- Capital 'T'
          ...
          <h4 className="text-lg font-medium mt-4 mb-2">Explanation & Fix:</h4>
          <p className="text-sm">
            The JSON literals for boolean and null values are strictly lowercase: <code>true</code>, <code>false</code>, and <code>null</code>.
            <br />
...
// This is not allowed after the array`}
          ...
          <h4 className="text-lg font-medium mt-4 mb-2">Explanation & Fix:</h4>
          <p className="text-sm">
            The formatter successfully parsed the array <code>[1, 2, 3]</code> but then found additional characters (which it correctly identified as the start of a comment, which is also invalid in JSON). Once the root element is closed, the input should end.
            <br />
            <strong>Fix:</strong> Remove any characters or content that appear after the main JSON object or array closes.
          </p>
          <div className="bg-white p-3 rounded dark:bg-gray-900 overflow-x-auto mt-4">
            <pre className="text-sm">
              {`[1, 2, 3]`}
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li>
            <strong>Locate the Error:</strong> Pay close attention to the line and column numbers provided in the error message. Most formatters highlight the exact location.
          </li>
          <li>
            <strong>Understand the Expectation:</strong> The error message often says what the parser *expected* to find but didn't. This is the key to identifying the missing or incorrect character.
          </li>
          <li>
            <strong>Fix One Error at a Time:</strong> Sometimes one syntax error can confuse the parser and lead to cascading, misleading error messages. Fix the first reported error, then re-validate.
          </li>
          <li>
