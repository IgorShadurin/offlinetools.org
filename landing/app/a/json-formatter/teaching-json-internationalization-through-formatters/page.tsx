...
"greetingWithUser": "¡Hola, {{userName}}!"`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li className="flex items-center gap-2">
            ...
            <strong>Dates and Times:</strong> Format (DD/MM/YYYY vs. MM/DD/YYYY), names of months/days, 12-hour vs. 24-hour clock.
          </li>
          <li className="flex items-center gap-2">
            <DollarSign className="w-5 h-5 text-green-500" />
            <strong>Numbers and Currencies:</strong> Decimal separators (<code>.</code> vs. <code>,</code>), thousands separators, currency symbols ($ vs. €), currency placement (before/after value), number of decimal places.
          </li>
          <li className="flex items-center gap-2">
            <RefreshCcw className="w-5 h-5 text-purple-500" />
            <strong>Relative Times:</strong> "just now", "2 minutes ago", "in 3 months". The phrasing and grammar depend on the duration and target language.
          </li>
          <li className="flex items-center gap-2">
            <LayoutList className="w-5 h-5 text-orange-500" />
            <strong>Plurals:</strong> How words change based on quantity (e.g., "1 item", "2 items", "0 items", and even more complex rules in languages like Arabic or Polish).
          </li>
          <li className="flex items-center gap-2">
            <Languages className="w-5 h-5 text-red-500" />
            <strong>Complex Messages:</strong> Messages where the structure changes significantly based on multiple variables or conditions (e.g., gender, item counts, etc.).
          </li>
        </ul>
        <p>
        ...
        <p>
          ...
          <a href="https://unicode-org.github.io/icu/userguide/format_parse/messages/" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400">ICU MessageFormat</a>, widely supported by i18n libraries.
        </p>
  ...
  "lastLogin": "Último inicio de sesión: {loginDate, date, medium}"
        ...
        <p>
          Here, <code>&#x7B;loginDate, date, medium&#x7D;</code> tells the formatter to take the value provided for <code>loginDate</code> and format it as a date using the "medium" style according to the current locale's rules.
        </p>
  ...
  "accountBalance": "Ihr Guthaben beträgt {balance, number, currency}."
        ...
        <p>
          <code>&#x7B;balance, number, currency&#x7D;</code> instructs the formatter to format the <code>balance</code> value as a number representing currency, respecting locale-specific rules for separators, currency symbol, etc.
        </p>
  ...
  "itemCount": "{itemCount, plural, one {# товар} few {# товара} many {# товаров} other {# товара}}"
        ...
        <p>
          The <code>&#x7B;itemCount, plural, ...&#x7D;</code> construct is powerful. It takes the <code>itemCount</code> and uses locale-specific plural rules to pick the correct phrase ("one", "few", "many", "other"). <code>#</code> is a special placeholder for the number itself, already formatted.
        </p>
        ...
        <p>
          The JSON file simply provides the template string. The actual formatting is done in the application code using built-in browser/Node.js APIs (like the <a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl" target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400"><code>Intl</code></a> object) or dedicated i18n libraries.
        </p>
        <p>
          Libraries like <code>react-intl</code> or <code>formatjs</code> parse the ICU MessageFormat strings from your JSON and use the browser's or a polyfill's <code>Intl</code> object to perform the locale-aware formatting.
        </p>

        <h3 className="text-xl font-semibold mt-6">Conceptual Code Example (using a hypothetical `translate` function):</h3>
        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
`}
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Reduced Translation Effort:</strong> Translators only need to provide the template string and the plural/select rules, not every single permutation of formatted data.
          </li>
          <li>
            <strong>Locale Correctness:</strong> Relies on established standards (like ICU) and built-in browser APIs (<code>Intl</code>) or robust libraries, ensuring correct date, number, and plural formatting for each locale.
          </li>
          <li>
            <strong>Code Simplicity:</strong> Application code passes raw data (numbers, dates, counts) to the translation function; the complexity of formatting is abstracted away.
          </li>
          <li>
            <strong>Flexibility:</strong> Easily add support for new locales; the formatting rules are already defined by the standard or library.
          </li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Separation of Concerns:</strong> JSON stores *what* needs to be displayed and the *rules* for dynamic parts; code handles the *how* (the formatting logic).
          </li>
          <li>
            <strong>The Importance of Locale:</strong> Formatting isn't just about inserting values; it's about doing so according to the user's specific language and region settings.
          </li>
          <li>
            <strong>Using Standard APIs/Libraries:</strong> Encourage the use of the <code>Intl</code> object or well-maintained i18n libraries that implement standards like ICU MessageFormat, rather than building custom formatting logic.
          </li>
          <li>
            <strong>Testing:</strong> Stress the importance of testing formatted messages with different data values (especially edge cases for plurals like 0, 1, 2, 5, 10, 20, 21, 100, 101 etc.) and in different locales.
          </li>
        </ul>
