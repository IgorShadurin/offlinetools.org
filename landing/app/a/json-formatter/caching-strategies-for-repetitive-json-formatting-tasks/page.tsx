...
`}
        ...
        <section>
          ...
          <p>
            The most straightforward caching strategy is <a href="https://en.wikipedia.org/wiki/Memoization" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">memoization</a>. We use the object instance itself as the key in a cache storage. When asked to stringify an object:
          </p>
          <ol className="list-decimal pl-6 space-y-2 my-4">
            ...
            <li>If no (cache miss), call <code>JSON.stringify()</code>, store the resulting string in the cache with the object reference as the key, and return the string.</li>
          </ol>
          <p>
            For the cache storage, <code><a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/WeakMap" target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400 hover:underline">WeakMap</a></code> is an excellent choice in JavaScript/TypeScript backend environments. Unlike a standard <code>Map</code>, <code>WeakMap</code> keys must be objects, and crucially, it holds "weak" references to these keys. If the object key is garbage collected because there are no other references to it, the corresponding entry in the <code>WeakMap</code> is also automatically removed. This prevents memory leaks.
          </p>
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
`}
        ...
        <section>
          ...
          <p>
            This is what the <code>WeakMap</code> example uses. The key is the specific instance of the object in memory.
          </p>
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li><span className="font-medium">Pros:</span> Extremely fast key lookup. Simple to implement. Avoids memory leaks with <code>WeakMap</code>. Ideal when the *same object instance* is passed repeatedly.</li>
            <li><span className="font-medium">Cons:</span> Fails if you have two different object instances with the *exact same content*. Does not handle mutation of the cached object effectively (see below).</li>
          </ul>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li><span className="font-medium">Pros:</span> More hits if identical data structures are generated independently.</li>
            <li><span className="font-medium">Cons:</span> Requires implementing or using a deep comparison/hashing function, which can be as computationally expensive (or more so) than <code>JSON.stringify()</code> itself, negating the benefit. Harder to manage the cache and memory. Generally not recommended for this specific task unless deep equality checking is already part of your workflow for other reasons.</li>
          </ul>
          <p>
            <span className="font-medium">Conclusion:</span> For caching <code>JSON.stringify</code> results, caching based on <span className="font-semibold">object identity using <code>WeakMap</code></span> is usually the most practical and performant approach in backend scenarios.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            The biggest pitfall of caching based on object identity is <span className="font-semibold text-red-600 dark:text-red-400">mutation</span>. If you cache the string for an object, and then later modify that object (e.g., change a property value, add/remove an item from an array within it), the cached string becomes <span className="font-semibold text-red-600 dark:text-red-400">stale</span>. Subsequent calls using the *same object reference* will return the old, incorrect JSON string.
          </p>
          <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
...
`}
          ...
          <p>
            <span className="font-medium">Solution:</span> This caching strategy is most effective when stringifying <span className="font-semibold">immutable data structures</span> or objects whose lifecycle ensures they are not mutated between the caching call and subsequent lookups within the cache's intended lifespan. If your objects are frequently mutated, this simple memoization strategy is <span className="font-semibold text-red-600 dark:text-red-400">not suitable</span>.
          </p>
        </section>
         ...
         <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <span className="font-medium">Static or Slow-Changing API Responses:</span> Caching the JSON output for endpoints serving data that is updated infrequently (e.g., configuration data, product lists with server-side rendering).
            </li>
            <li>
              <span className="font-medium">Server-Side Rendering (SSR) of Stable Data:</span> If parts of the data structure used for SSR are static or only change with deployments, caching the formatted JSON for those parts can reduce render time.
            </li>
            <li>
              <span className="font-medium">Pure Functions Returning Objects:</span> Caching the output of a function that always returns the same object instance (or a deep clone that is treated as immutable within its usage context) for the same inputs.
            </li>
            <li>
              <span className="font-medium">Data Processed in Batches:</span> If the same objects are processed and formatted multiple times within a single batch job or request lifecycle where you know they won't be mutated.
            </li>
          </ul>
        </section>
        ...
        <section>
          ...
          <ul className="list-disc pl-6 space-y-2 my-4">
            <li>
              <span className="font-medium">Highly Dynamic Data:</span> Objects that change frequently based on user interaction, database updates, or external events.
            </li>
            <li>
              <span className="font-medium">Objects with Short Lifespans:</span> If objects are created and discarded quickly, the overhead of cache checking and setting might outweigh the benefit.
            </li>
            <li>
              <span className="font-medium">When Object Mutation is Unavoidable:</span> Unless you have a robust invalidation strategy tied to mutations (which is complex), avoid caching mutable objects by reference.
            </li>
            <li>
              <span className="font-medium">Small Objects:</span> For tiny objects, <code>JSON.stringify</code> is usually fast enough that caching provides negligible benefit.
            </li>
          </ul>
        </section>
