  ...
  return (
    <article className="container mx-auto px-4 py-8 max-w-3xl">
      ...
      <div className="space-y-6 text-lg text-gray-800 dark:text-gray-200">
        ...
        <p>
          <strong>JSON (JavaScript Object Notation)</strong> is a lightweight, human-readable data interchange format that has become a ubiquitous choice for configuration files due to its simplicity, flexibility, and wide support across programming languages.
        </p>
        ...
        <ul className="list-disc pl-6 space-y-2">
          ...
          <li>
            <strong>Hierarchical Structure:</strong> It naturally supports nested configurations, organizing settings logically.
          </li>
          <li>
            <strong>Language Agnostic:</strong> Parsers are available for virtually every programming language.
          </li>
          <li>
            <strong>Schema Flexibility:</strong> While not strictly schema-enforced by the format itself, its structure is straightforward. (Tools can enforce schemas like JSON Schema).
          </li>
          <li>
...
// instead of accessing it globally like this.`}
        ...
        <ul className="list-disc pl-6 space-y-2">
           <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Simple to implement.</li>
           <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Easy for development and testing locally.</li>
        </ul>

        <h3 className="text-xl font-semibold mt-6 mb-3">Cons of Local Files:</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Requires redeployment to change config.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Configuration spread across potentially many services.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Difficult for dynamic, runtime updates.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Sensitive data (like passwords) should never be stored here.</li>
        </ul>
        ...
        <ol className="list-decimal pl-6 space-y-2">
          <li>Configuration (often JSON, YAML, etc.) is stored centrally (e.g., Git repository, database, dedicated server's backend).</li>
          <li>The configuration server exposes an API for services to retrieve config.</li>
          <li>A microservice starts up and makes a request to the config server to get its relevant configuration, often identified by service name, environment (dev, staging, prod), and possibly version.</li>
          <li>The service parses the received configuration (which could be JSON) and applies the settings.</li>
          <li>Some systems support dynamic updates where the service is notified of config changes and reloads them without a restart.</li>
        </ol>

        <h3 className="text-xl font-semibold mt-6 mb-3">Pros of Centralized Servers:</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Single source of truth for configuration.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Easier management of configurations across many services and environments.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Supports dynamic configuration updates.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Better integration with secrets management.</li>
        </ul>

        <h3 className="text-xl font-semibold mt-6 mb-3">Cons of Centralized Servers:</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Adds another dependency (the config server itself).</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Requires implementing client-side logic in each service to fetch/listen for config.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Can introduce complexity in managing the config server infrastructure.</li>
        </ul>
...
// loggingLevel becomes "INFO"`}
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Standardized across different deployment platforms.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Good for passing simple values or overriding JSON values.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Excellent for non-sensitive values that vary by environment.</li>
        </ul>

        <h3 className="text-xl font-semibold mt-6 mb-3">Cons of Environment Variables:</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Not suitable for complex, structured configurations.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Can become unwieldy with many variables.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Sensitive data should still be handled carefully, often requires integration with secrets management.</li>
        </ul>
...
// 3. Use this key to fetch the actual password from the secrets manager API.`}
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Securely stores and retrieves sensitive data.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Auditing of secret access.</li>
          <li><Check className="inline h-5 w-5 text-green-500 mr-1" /> Supports rotation of secrets.</li>
        </ul>

         <h3 className="text-xl font-semibold mt-6 mb-3">Cons of Secrets Management:</h3>
        <ul className="list-disc pl-6 space-y-2">
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Adds complexity to service startup and operational overhead for the secrets manager itself.</li>
          <li><X className="inline h-5 w-5 text-red-500 mr-1" /> Requires proper access control configuration.</li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <strong>Default Configuration:</strong> A base JSON file with settings that are common across environments or provide sensible defaults.
          </li>
          <li>
            <strong>Environment-Specific Overrides:</strong> JSON snippets or files specific to 'dev', 'staging', 'prod' that override defaults, often fetched from a central config server.
          </li>
          <li>
            <strong>Environment Variables:</strong> Used for simple overrides or deployment-specific settings (like the config server URL itself).
          </li>
          <li>
            <strong>Secrets Management:</strong> For all sensitive credentials and keys, referenced by configuration but stored externally and securely.
          </li>
        </ul>
        <p>
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <strong>Keep it Structured:</strong> Use nested objects to group related settings (e.g., all database settings under a "database" key).
          </li>
          <li>
            <strong>Consistency:</strong> Define a consistent structure across services where possible, even if they don't use all fields. This aids understanding and tooling.
          </li>
           <li>
            <strong>Avoid Redundancy:</strong> Don't repeat the same configuration values across many services if they can be externalized and shared (e.g., common service discovery settings).
          </li>
          <li>
            <strong>Comments:</strong> Standard JSON doesn't support comments. If you need documentation within the config file, consider using a format that supports it (like HJSON or JSONC, then converting to standard JSON) or storing documentation separately. Many config servers support formats with comments (like YAML) and can serve JSON.
          </li>
          <li>
            <strong>Validation:</strong> Use JSON Schema or similar tools to validate configuration files against a defined structure before deploying. This catches errors early.
          </li>
        </ul>
        ...
        <ul className="list-disc pl-6 space-y-2">
          <li>
            <strong>YAML:</strong> Often preferred for its human-friendliness and support for comments, anchors, and aliases, making it concise for complex structures.
          </li>
          <li>
            <strong>TOML:</strong> Simpler than YAML, designed to be easy to read due to obvious semantics.
          </li>
          <li>
            <strong>Environment Variables:</strong> As discussed, often used alongside other formats for simple values and overrides.
          </li>
          <li>
