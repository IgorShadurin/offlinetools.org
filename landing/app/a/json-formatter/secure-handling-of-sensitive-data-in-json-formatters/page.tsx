...
const Icon = ({ IconComponent, size = 24, className = "" }: { IconComponent: React.ElementType, size?: number, className?: string }) => (
    ...
    <>
      ...
      <div className="space-y-6">
        ...
        <ul className="list-disc pl-6 space-y-2 my-4">
          <li>
            <strong>Accidental Inclusion:</strong> Developers might inadvertently include sensitive fields (e.g., a user's password hash from the database object) when serializing data to JSON for an API response or a log file.
          </li>
          <li>
            <strong>Detailed Error Messages:</strong> Error responses formatted as JSON might expose internal details, including snippets of data or database query information that contain sensitive values.
          </li>
          <li>
            <strong>Logging and Monitoring:</strong> Server-side logs, application monitoring tools, or request/response logging middleware often capture JSON payloads. If sensitive data is included, it becomes permanently stored and accessible to anyone with access to the logs. <Icon IconComponent={Server} className="text-blue-500" />
          </li>
          <li>
            <strong>Client-Side Persistence (Mention):</strong> Although this is a server page, data sent from the server can be stored or processed insecurely on the client (browser history, local storage, console logs) if the server didn't redact it first.
          </li>
          <li>
            <strong>Clipboard Exposure (Mention):</strong> If a user copies JSON data (e.g., from developer tools or a UI element displaying raw JSON) that contains sensitive information, it sits in the clipboard and can potentially be accessed by other applications.
          </li>
        </ul>
        ...
        <p>
          <Icon IconComponent={EyeOff} className="text-green-600" /> The primary technique is to remove or mask sensitive fields from data objects *before* serializing them into a JSON string. This should be applied to API responses, data being stored in logs, or any other output format.
        </p>
        <p>
          Implement functions that take a data object and return a new object with sensitive fields either removed, replaced with a placeholder (like <code>"********"</code>), or partially masked (e.g., showing only the last four digits of a credit card number).
        </p>

        <div className="bg-gray-100 p-4 rounded-lg dark:bg-gray-800 my-4">
          <h3 className="text-lg font-medium flex items-center"><Icon IconComponent={EyeOff} />Example: Redacting an Object Before JSON Serialization</h3>
...
`}
          ...
          <p className="mt-3 text-sm text-gray-700 dark:text-gray-300">
            Implement the <code>redactSensitiveData</code> function to handle different data types and recursive structures (nested objects and arrays). Maintain a list of keys considered sensitive.
          </p>
        </div>
...
`}
        ...
        <p>
          Securing sensitive data in JSON formatters is a critical aspect of backend development. By understanding the risks of accidental exposure and implementing robust server-side techniques like data redaction, careful data selection, and secure logging, developers can significantly mitigate the chances of data leaks. Always err on the side of caution: if data doesn't need to be exposed in JSON, remove it or mask it before serialization. <Icon IconComponent={Shield} className="text-blue-500" />
        </p>
      </div>
    </>
  );
